...
| Git tags | tags `v1.0.0`, `v2.0.0` | Annotated release tags marking the feature-complete builds described in CHANGELOG. |
| Release APK asset | `.gitlab-ci.yml` (`build-apk`, `release-apk` jobs), `README.md` (install section), `mobile/eas.json` | On every version tag, CI builds the Android APK (EAS `preview` profile → `apk`), uploads it to the project's generic Package Registry, and creates a GitLab Release with the APK attached as a linked asset. Release notes and the README both carry step-by-step install instructions. Requires a masked `EXPO_TOKEN` CI/CD variable. |
