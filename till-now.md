...
| Commit | What |
|---|---|
| UI v2 | **Visual overhaul, both apps**: "paper & ink" theme (warm paper / emerald-teal / saffron-gold) with full **light/dark toggle**; Fraunces+Manrope typography; styled **"Billology" wordmark splash** (fades in→out, no emojis); landing redesigned with drawn receipt graphic + buttons-only capture (paste box hidden until tapped); **in-app multi-shot camera** (UPI-style bottom shutter, up to 5 pages); bar-graph y-axis on standard 1/2/5×10ⁿ steps; **missing-date prompt** (read off the bill photo / set manually); **Help & guide** section (Settings → mobile, ? button → web). New i18n keys reviewed for en/hi/te, others fall back to English. |
